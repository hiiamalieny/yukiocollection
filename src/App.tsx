...
const App: React.FC = () => {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center text-yukio-text font-bold animate-pulse">
        載入中...
      </div>
    );
  }
        ...
        <div className="bg-white rounded-3xl p-6 mb-8 shadow-sm border border-white flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            ...
            <p className="text-yukio-text/60 text-sm">
              加油！仲有{' '}
              <span className="font-bold text-yukio-text">{totalItems - ownedCount}</span> 隻就儲齊啦！
            </p>
          </div>
